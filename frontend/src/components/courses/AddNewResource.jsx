...
const AddNewResource = ({ lessonId, onClose, fetchCourses }) => {
  ...
  const handleSubmit = async () => {
    try {
        console.log('inside submit of new resource form');
      const response = await axiosInstance.post(
        `/api/lessons/addResource/${lessonId}`,
        { title, type, instructorId: id }
      );
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60">
      <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] max-w-xl relative">
        <span className="flex gap-x-2 items-center mb-6">
          ...
          <h2 className="text-2xl font-medium text-left">New Resource</h2>
        </span>
