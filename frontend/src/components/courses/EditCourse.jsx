...
const EditCourse = ({ onClose, fetchCourses, course }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/api/courses/update/${course._id}`, {
        title,
        description,
        category,
        level,
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60">
      <div className="bg-white p-8 rounded-lg shadow-lg w-[600px] max-w-xl relative">
        <span className="flex gap-x-2 items-center mb-6">
          ...
          <h2 className="text-2xl font-medium text-left">Edit Course</h2>
        </span>
